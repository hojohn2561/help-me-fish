...
function getSpecificHelpIntro(
  ...
  let helpStr = `Fishing for atlantic croaker when the water temperature is ${waterTemperature}°F `;
  ...
  helpStr += `Water clarity isn't that important when targeting croakers.`;

  return helpStr;
}

function getSpecificLures() {
  const { atlanticCroaker } = constants.species;

  let luresInfo = {
    intro: saltwaterFishLureStrings[atlanticCroaker].intro,
    types: {},
  };

  // Bait/Lure names
  const { fishbites, shrimp, worms } = constants.lures;

  // Strings describing the lures
  const { general: fishbitesStr } =
    saltwaterFishLureStrings[atlanticCroaker][fishbites];
  const { general: shrimpStr } =
    saltwaterFishLureStrings[atlanticCroaker][shrimp];
  const { general: wormsStr } =
    saltwaterFishLureStrings[atlanticCroaker][worms];

  let lureTypes = {};

  // Choosing which description to display, based on the user's input to the form
  lureTypes[fishbites] = {
    message: fishbitesStr,
    image: checkmark,
  };
  lureTypes[shrimp] = {
    message: shrimpStr,
