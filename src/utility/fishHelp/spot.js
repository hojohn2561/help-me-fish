...
function getSpecificHelpIntro(
  ...
  let helpStr = `Fishing for spot when the water temperature is ${waterTemperature}°F `;
  ...
  helpStr += `Water clarity isn't that important when targeting spot.`;

  return helpStr;
}

function getSpecificLures() {
  const { spot } = constants.species;

  let luresInfo = {
    intro: saltwaterFishLureStrings[spot].intro,
    types: {},
  };
  ...
  const { general: fishbitesStr } = saltwaterFishLureStrings[spot][fishbites];
  const { general: shrimpStr } = saltwaterFishLureStrings[spot][shrimp];
  const { general: wormsStr } = saltwaterFishLureStrings[spot][worms];
